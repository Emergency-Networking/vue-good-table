<template>
	<div :class="wrapStyleClasses">
		...
		<div class="vgt-inner-wrap" :class="{ 'is-loading': isLoading }">
			<slot v-if="paginate && paginateOnTop" name="pagination-top" :pageChanged="pageChanged" :perPageChanged="perPageChanged" :total="totalRows || totalRowCount">
				<vgt-pagination
					ref="paginationTop"
			...
			<vgt-global-search @on-keyup="searchTableOnKeyUp" @on-enter="searchTableOnEnter" v-model="globalSearchTerm" :search-enabled="searchEnabled && externalSearchQuery == null" :global-search-placeholder="searchPlaceholder">
				<template slot="internal-table-actions">
					<slot name="table-actions"> </slot>
				</template>
			</vgt-global-search>
			<div v-if="selectedRowCount && !disableSelectInfo" class="vgt-selection-info-row clearfix" :class="selectionInfoClass">
				{{ selectionInfo }}
					...
					<tbody v-for="(headerRow, hIndex) in paginated" :key="hIndex">
						...
						>
							<template v-if="hasHeaderRowTemplate" slot="table-header-row" slot-scope="props">
								<slot name="table-header-row" :column="props.column" :formattedRow="props.formattedRow" :row="props.row"> </slot>
							</template>
						</vgt-header-row>
						...
						>
							<th v-if="lineNumbers" class="line-numbers" :aria-label="`Row ${getCurrentIndex(row.originalIndex)}`">
								{{ getCurrentIndex(row.originalIndex) }}
							</th>
							<th v-if="selectable" @click.stop="onCheckboxClicked(row, index, $event)" class="vgt-checkbox-col" :aria-label="`Select Row ${getCurrentIndex(row.originalIndex)} Cell`">
								<input type="checkbox" :disabled="row.vgtDisabled" :checked="row.vgtSelected" :aria-label="`Select Row ${getCurrentIndex(row.originalIndex)}`" />
							</th>
							<td
			...
			<slot v-if="paginate && paginateOnBottom" name="pagination-bottom" :pageChanged="pageChanged" :perPageChanged="perPageChanged" :total="totalRows || totalRowCount">
				<vgt-pagination
...
export default {
	...
	methods: {
		...
		initializeSearch() {
			const { enabled, trigger, externalQuery, searchFn, placeholder, skipDiacritics } = this.searchOptions;
		...
		initializeSelect() {
			const { enabled, selectionInfoClass, selectionText, clearSelectionText, selectOnCheckboxOnly, selectAllByPage, disableSelectInfo, selectAllByGroup } = this.selectOptions;
